...
Between the Proposers and the live orders sits a governance layer. A three-condition VIX gate can reduce overall exposure if volatility spikes above thresholds and holds there for two or more consecutive days. A separate reduce_50pct flag cuts mean-reversion capital in half during elevated regimes. A dead-band filter suppresses small rebalancing trades below a minimum threshold—specifically, trades that would cost more in spread and slippage than they could recover in expected signal. Finally, position caps prevent any single name from dominating a sleeve's risk budget.
