[Trend_Following_Project_README.md](https://github.com/user-attachments/files/29111101/Trend_Following_Project_README.md)
# Building a Smoother Ride: What I Learned Designing a Trend-Following Strategy

> **Status: Paper trading live as of June 18, 2026.** Seven positions entered, automated daily monitoring and monthly rebalancing running via Windows Task Scheduler on Interactive Brokers paper account. First rebalance scheduled July 28, 2026.
...
**Paper trading is live.** On June 18, 2026, the strategy entered its first real positions on an Interactive Brokers paper account with ~$994,000 CAD in starting capital. Seven ETF positions were opened across US equities and international markets, matching the weights the signal engine computed from that day's closing data.

| Ticker | Shares | Entry value |
|--------|--------|-------------|
| IWM | 872 | $256,787 |
| EEM | 3,221 | $227,951 |
| SMH | 318 | $209,727 |
...
The live execution pipeline runs automatically: prices refresh daily after market close, NAV and P&L are logged, and the strategy rebalances monthly on the last trading day. The first scheduled rebalance is July 28, 2026.

The key question paper trading answers that backtesting cannot: do the real fill prices, data feeds, and order execution match the assumptions built into the backtest? A follow-up writeup comparing live results to backtest expectations will be added here as data accumulates.

---

## The live execution stack

Five Python scripts run the strategy end to end:

- **`data_feed.py`** — pulls 3 years of daily price history for all 12 assets from Interactive Brokers, saves locally
- **`signal_engine.py`** — computes TSMOM and CS-Mom signals, regime filter, and fast-exit trigger from the local price data
- **`position_sizer.py`** — converts signals to target portfolio weights using vol-targeting and the equity/diversifier caps
- **`order_engine.py`** — reads target weights, sizes orders in shares, applies the tiered dead-band, and submits to IBKR
- **`monitor.py`** — logs daily NAV and P&L, saves current weights for the next rebalance's dead-band comparison
